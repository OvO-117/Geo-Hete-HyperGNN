...
dataname='esol'
model_dict=torch.load('/home/dt3/桌面/XZ/visial_chh/.optuna_runs/chhtrans_optuna_regress/esol_final/trial_1/chhtrans_experiment_20260116_211915/models/esol_final_best_model_epoch_260.pth')
...
NUMBER_TO_SYMBOL = {
        ...
        25: "Mn", 26: "Fe", 27: "Co", 28: "Ni", 29: "Cu", 30: "Zn", 34: "Se",
        35: "Br", 53: "I",
}
...
def transform_scores(scores, lower_pct=2.0, upper_pct=98.0, scale=None, method='asinh', gamma=0.6):
    ...
    if method == 'asinh':
        ...
        y = ranks.astype(float) / (n - 1 if n > 1 else 1.0)
    else:
        y = s
    ymin = y.min()
    ymax = y.max()
...
# dataset=dataset[705:707]
for choice_id in range(len(dataset)):
    try:
        ...
        sample_dir = os.path.join('photo', dataname, str(choice_id))
        ...
        for block in range(num_blocks):
            ...
            raw_scores_blocks.append(scores)
        methods = ['asinh','log1p','signed_log','rank']
        for method in methods:
            ...
            transformed_blocks = [transform_scores(raw, lower_pct=2.0, upper_pct=98.0, method=method, gamma=0.6) for raw in raw_scores_blocks]
            g_min = float(min([blk.min() for blk in transformed_blocks]))
            g_max = float(max([blk.max() for blk in transformed_blocks]))
            for block in range(num_blocks):
                scores = transformed_blocks[block]
                bnorm = mcolors.Normalize(vmin=g_min, vmax=g_max)
                normed = (scores - g_min) / (g_max - g_min + 1e-12)
                ...
                hnx.draw(
                    ...
                    edge_labels_kwargs={'fontsize':40,'fontweight':'bold'},
                ...
                ax.set_title(f"Block {block}", fontsize=100, fontweight='bold', pad=0, y=0.7)
            ...
            cbar.set_label('Node Attention Score', fontsize=80, fontweight='bold')
            ...
            edge_cmap_sample_ax.set_xlabel('Edge Attention Score', fontsize=80, fontweight='bold')
            edge_cmap_sample_ax.tick_params(axis='x', labelsize=30)
            plt.tight_layout(rect=[0,0,1,0.95], pad=0.2)
            plt.savefig(os.path.join(sample_dir, f'{method}.png'))
            plt.close(fig)
    except Exception as e:
        print(f'choice_id {choice_id} error: {e}')
        continue