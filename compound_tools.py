...
def mol_to_geognn_graph_data(mol, atom_poses):
    """
    mol: rdkit molecule
...
def mol_to_geognn_graph_data_MMFF3d(mol,numConfs=100):
    """tbd"""
    if len(mol.GetAtoms()) <= 400:
        print('use MMFF3d')
        mol, atom_poses = Compound3DKit.get_MMFF_atom_poses(mol, numConfs)
    else:
        print('use 2d')
        atom_poses = Compound3DKit.get_2d_atom_poses(mol)
    return mol_to_geognn_graph_data(mol, atom_poses),atom_poses


def mol_to_geognn_graph_data_raw3d(mol):
    """tbd"""
    atom_poses = Compound3DKit.get_atom_poses(mol, mol.GetConformer())
    return mol_to_geognn_graph_data(mol, atom_poses)
