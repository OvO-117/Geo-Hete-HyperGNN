...
def train(model, train_loader, optimizer, loss_fn, args, loss_weight=0.5):
    ...
    for batch in train_loader:
        ...
        if args.num_classification is not None:
            ...
            predict = predict # 确保预测值的维度正确
            loss = loss_fn(predict, target)
            total_y.append(target.cpu())
            # print(predict)
            total_predictions.append(predict.detach().cpu())

        # total_loss = loss_weight * loss + (1 - loss_weight) * bt_loss
        total_loss = loss_weight * loss + (1 - loss_weight) * args.bt_loss_scale * bt_loss
    ...
    if args.num_classification is not None:
        ...
        y_probs = torch.cat(total_pred_probs).numpy()
        try:
        ...
        return epoch_loss/num_loss, acc, auc, total_y, total_pred_probs, total_bt_loss/num_loss
    else:
        # 回归任务评价指标
        y_true = torch.cat(total_y).numpy()
        y_pred = torch.cat(total_predictions).numpy()
        
        # 计算MSE、RMSE和MAE（与RMSE来源一致）
        ...
        return mse, rmse, mae, r2, total_y, total_predictions, total_bt_loss/num_loss
...
def test(model, test_dataloader, args):
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            if args.num_classification is not None:
                ...
                total_y.append(target.cpu())
            # print('target_len',len(total_y))    
            # print('predict_len',len(total_y))
    ...
    if args.num_classification is not None:
        ...
        return epoch_loss/num_loss, acc, auc, total_y, total_pred_labels, bt_loss.item()
    else:
        # 回归任务评价指标
        y_true = torch.cat(total_y).numpy()
        y_pred = torch.cat(total_predictions).numpy()
...
def main():
    ...
    # 数据参数
    # parser.add_argument('--data_path', type=str, default='./data/bace', help='数据集路径')
    parser.add_argument('--dataset_name', type=str, default='lipo_final', help='数据集名称')
    ...
    # parser.add_argument('--ratio', type=float, default=0.8, help='比例参数')
    ...
    parser.add_argument('--loss_weight', type=float, default=0.6, help='损失权重 (CE/总损失权重)')
    parser.add_argument('--bt_loss_scale', type=float, default=0.05, help='BT损失缩放系数，用于减小其影响')
    parser.add_argument('--eta_min_ratio', type=float, default=0.01, help='学习率调度器最小比例 (eta_min=lr*ratio)')
    parser.add_argument('--num_classification', type=int, default=None, help='分类类别数，设为None进行回归任务')

    # 数据分割参数
    parser.add_argument('--train_ratio', type=float, default=0.8, help='训练集比例')
    parser.add_argument('--random_seed', type=int, default=225, help='随机种子')
    ...
    parser.add_argument('--save_model', default='/home/ubuntu/桌面/AW/moe_chhtrans/model_check_point', help='是否保存最佳模型')
    ...
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=args.batch_size, num_workers=4, pin_memory=True, worker_init_fn=_seed_worker, generator=torch.Generator(device='cpu').manual_seed(args.random_seed))
    valid_loader = DataLoader(valid_dataset, shuffle=False, batch_size=args.batch_size, num_workers=4, pin_memory=True, worker_init_fn=_seed_worker, generator=torch.Generator(device='cpu').manual_seed(args.random_seed))
    test_loader = DataLoader(test_dataset, shuffle=False, batch_size=args.batch_size, num_workers=4, pin_memory=True, worker_init_fn=_seed_worker, generator=torch.Generator(device='cpu').manual_seed(args.random_seed))
    ...
    with open(log_file, 'w') as f:
        ...
        f.write("\n训练过程:\n")
        if args.num_classification is not None:
            f.write("Epoch\tTrain_AUC\tTest_AUC\tTrain_CE\tTest_CE\n")
        else:
            f.write("Epoch\tTrain_RMSE\tTrain_MAE\tTrain_R2\tTest_RMSE\tTest_MAE\tTest_R2\tTrain_MSE\tTest_MSE\n")
    ...
    for epoch in all_epochs:
        if args.num_classification is not None:
            # 分类任务
            train_ce, train_acc, train_auc, train_trues, train_probs= train(model, train_loader, optimizer, loss_fn, args, args.loss_weight)
            ...
            log_info = f'{epoch}\t{train_auc:.4f}\t{test_auc:.4f}\t{train_ce:.4f}\t{test_ce:.4f}'
            print(f'epoch:{epoch}, train_AUC:{train_auc:.4f}, test_AUC:{test_auc:.4f}, train_CE:{train_ce:.4f},  test_CE:{test_ce:.4f}')
            ...
            # 回归任务
            train_mse, train_rmse, train_mae, train_r2, train_trues, train_preds,train_bt_loss= train(model, train_loader, optimizer, loss_fn, args,args.loss_weight)
            ...
            log_info = f'{epoch}\t{train_rmse:.4f}\t{train_mae:.4f}\t{train_r2:.4f}\t{test_rmse:.4f}\t{test_mae:.4f}\t{test_r2:.4f}\t{train_mse:.4f}\t{test_mse:.4f}\t{train_bt_loss:.4f}'
            print(f'epoch:{epoch}, train_RMSE:{train_rmse:.4f}, train_MAE:{train_mae:.4f}, train_R2:{train_r2:.4f}, test_RMSE:{test_rmse:.4f}, test_MAE:{test_mae:.4f}, test_R2:{test_r2:.4f}, train_MSE:{train_mse:.4f}, test_MSE:{test_mse:.4f}, train_BT_loss:{train_bt_loss:.4f}')
    ...
    if args.save_model and best_model_state is not None:
        model_path = os.path.join(models_dir, f"{args.dataset_name}_best_model_epoch_{best_epoch}.pth")
        save_dict = {
            'epoch': best_epoch,
